(function () {
  ...
  var SHAPES = {
    ...
    screen:  { group:"print", make:function(){ return {size:0.55, edge:0.62, soft:0.85,
                                                       pitch:72, warp:0.70, patch:0.35}; } },
  ...
  var STYLES = [
    ...
    ["Screen print",    [{ t:"screen", r:"design" }], "print"]
  ];
  ...
  /* A halftone screen. The shape underneath is never printed flat — only
     as dots that grow and shrink with it, so the colour closes to solid
     where the shape is strongest and opens to bare cloth where it is not.
     A soft lattice of diamonds under a fine dot grid is the screened race
     vest: the dots are what you see, the diamonds are what they add up to. */
  function screenCanvas(el) {
    return cached("sr|" + el.colour + "|" +
                  [el.size, el.edge, el.soft, el.pitch, el.warp, el.patch, el.seed].join("|"),
      function () {
        var w = 1600, h = Math.max(2, Math.round(1600 * TH / TW));
        var c = document.createElement("canvas");
        c.width = w; c.height = h;
        var x = c.getContext("2d");
        var noise = makeNoise(el.seed);
        x.fillStyle = el.colour;
        /* u spans both panels, so stretch it by the texture aspect and the
           lattice comes out square on the body rather than squashed */
        var aspect = TW / TH;
        var freq = Math.max(1, 1 / Math.max(0.08, el.size));
        var soft = Math.max(0.05, el.soft);
        /* the dot pitch is counted per panel, and there are two of them */
        var cell = Math.max(2.5, w / (el.pitch * 2));
        for (var gy = cell / 2; gy < h; gy += cell) {
          for (var gx = cell / 2; gx < w; gx += cell) {
            var u = gx / w * aspect, v = gy / h;
            var wu = u + (noise(u * 1.7, v * 1.7, 3) - 0.5) * el.warp * 0.30;
            var wv = v + (noise(u * 1.7 + 7, v * 1.7 + 5, 3) - 0.5) * el.warp * 0.30;
            var a = (wu + wv) * freq, b = (wu - wv) * freq;
            var da = Math.abs(a - Math.floor(a) - 0.5);
            var db = Math.abs(b - Math.floor(b) - 0.5);
            /* 0 at the middle of a diamond, 1 out at its corner */
            var d = Math.max(da, db) * 2;
            var t = 0.5 + (el.edge - d) / soft;
            t = t < 0 ? 0 : t > 1 ? 1 : t;
            t = smooth(t);
            /* some diamonds print heavier than others, the way a screen
               lays down unevenly across a panel */
            t *= 1 - el.patch * noise(wu * 0.9 + 21, wv * 0.9 + 13, 2);
            if (t <= 0.01) continue;
            /* dots touch and then overlap as the tone closes up */
            var r = cell * 0.80 * Math.sqrt(t);
            if (r < 0.35) continue;
            x.beginPath();
            x.arc(gx, gy, r, 0, Math.PI * 2);
            x.fill();
